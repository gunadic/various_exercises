...
twoD_array_t * build_array(twoD_array_t * a, int rows, int cols) {
    ...
    for (row = 0; row < rows; ++row, temp+=cols) {
        a->elems[row] = temp;
    }
    return a;
}
...
void update_board(twoD_array_t *board, twoD_array_t *new_board) {
    int r, c, tally=0;
    for (r = 0; r < board->rows-1; ++r) {
        for (c=0; c < board->cols-1; ++c){
            //printf("Column %d Row %d\n", c, r);
            //printf("Number of columns %d\n", board->cols);
            //printf("Number of rows: %d\n", board->rows);
            if(c>0 && c<(board->cols-1) && r>0 && r<(board->rows-1)){
                /*Makes sure to avoid the borders */
                tally=0;
                if(board->elems[r-1][c-1]==1 )
                    tally++;
                if(board->elems[r][c-1]==1 )
                    tally++;
                if(board->elems[r+1][c-1]==1 )
                    tally++;
                if(board->elems[r-1][c]==1)
                    tally++;
                if(board->elems[r+1][c]==1)
                    tally++;
                if(board->elems[r-1][c+1]==1)
                ...
                if(board->elems[r+1][c+1]==1)
                    tally++;
                if(board->elems[r][c]==1){
                        if(tally==2 || tally==3)
                        new_board->elems[r][c]=1;
                    else
                        new_board->elems[r][c]=0;
                }
                if(board->elems[r][c]==0){
                    if(tally==3){
                        new_board->elems[r][c]=1;
                    }
                    else{
                        new_board->elems[r][c]=board->elems[r][c];
                    }
                }
            //  printf("tally for cell in row %d column %d is %d.\n", r, c, tally);

                // TODO: Figure out a more elegant way to read all of the bits and pieces around a given cell.
            }
        }
     }
   /* for (r = 0; r < board->rows; ++r) {
